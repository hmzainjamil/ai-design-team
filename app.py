...
from streamlit_patches import patch_streamlit_websocket
...
# Load WebSocket fix script
with open("static/js/websocket_fix.js") as f:
...
def configure_websocket_ssl():
    """Configure WebSocket to use SSL/TLS"""
    try:
        if not hasattr(st, '_server'):
            return
        ...
        if isinstance(server, Server):
            # Override the WebSocket URL generation
            original_get_url = server.get_websocket_url

            def get_websocket_url(*args, **kwargs):
                url = original_get_url(*args, **kwargs)
                # Force WSS protocol
                url = url.replace('ws://', 'wss://')
                return url

            server.get_websocket_url = get_websocket_url

            # Update WebSocket handler configuration
            if hasattr(server, 'websocket_handler'):
                handler = server.websocket_handler
                # Force secure headers
                handler.set_default_headers = lambda: handler.set_header('X-Forwarded-Proto', 'https')
        ...
        print(f"WebSocket SSL configuration failed: {str(e)}")
        pass

# Remove port settings from environment
for port_var in ['STREAMLIT_SERVER_PORT', 'PORT']:
    if port_var in os.environ:
        del os.environ[port_var]
